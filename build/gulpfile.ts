...
export const copyFiles: TaskFunction = async (done) =>{
    let global = resolve(rootPath, "types", "global.d.ts");
    return Promise.all([
...
export const copyTypeDefine: TaskFunction = async (done) => {
    return Promise.all([
        await excute(`rsync -a ${distPath}/types/ ${buildConfig['cjs'].output.path}/`),
        await excute(`rsync -a ${distPath}/types/ ${buildConfig['esm'].output.path}/`)
    ])
}
